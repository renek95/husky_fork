...
public class ConvenienceCommunication extends CamelService {
    ...
    /**
     * Determines if SubmissionSet metadata will be extracted automatically (e.g.
     * from CDA documents)
     */
    ...
    /**
     * Instantiates a new convenience communication without affinity domain set-up.
     * ATNA audit is disabled (unsecure)
     */
    ...
    /**
     * Instantiates a new convenience communication with the given affinity domain
     * set-up. ATNA audit is disabled (unsecure)
     *
    ...
    public void addDocumentToFolder(String documentEntryUUID, String folderEntryUUID) {
        var association = new Association(AssociationType.HAS_MEMBER, UUID.randomUUID().toString(), folderEntryUUID,
                documentEntryUUID);
    ...
    public FolderMetadata addFolder(Code submissionSetContentType) {
        ...
        txnData.getSubmissionSet().setContentTypeCode(XdsMetadataUtil.convertEhcCodeToCode(submissionSetContentType));
    ...
    /**
     * Cda fixes of CDAExtraction bugs and extraction methods, which are unsafe,
     * because an XDS registry might use another value set.
     *
    ...
    private void cdaExtractionFixes(DocumentMetadata docMetadata) {
        ...
        // Checks if the unique ID is longer than 64 or if no unique ID is set a new
    ...
    /**
     * creates an XDM volume with default values. You have to add a document to this
     * class first.
     *
    ...
    /**
     * creates an XDM volume with a given XdmContents object. This method will be
     * used, if you want to create your own INDEX.HTM and README.TXT for your XDM
     * volume. You have to add a document to this class first.
     *
     * @param outputStream The outputStream object where the contents will be
     *                     written to.
    ...
    /**
     * creates an XDM volume with the given submission set metadata. You have to add
     * a document to this class first.
     *
    ...
    private void linkDocumentEntryWithSubmissionSet() {
        ...
        for (Folder folder : txnData.getFolders()) {
            ...
            var association = new Association(AssociationType.HAS_MEMBER, txnData.getSubmissionSet().getEntryUuid(),
                    txnData.getSubmissionSet().getEntryUuid(), folder.getEntryUuid());
            association.assignEntryUuid();
    ...
    /**
     * Queries the document registry of the affinity domain for documents, using a
     * find documents query. This is useful if the number of results is limited in
     * the registry and your query would exceed this limit. In this case, precise
     * your query or do a query for references first, choose the possible matches
     * (e.g. the last 10 results) and then query for metadata.
     *
     ...
     *                       org.projecthusky.communication.storedquery and
    ...
    /**
     * Submission of the previously prepared document(s) to the repository<br>
    ...
    /**
     * Submission of the previously prepared document(s) to the repository to
     * replace another document. The restriction of this method is that only one
     * document could be replaced<br>
     * IHE [ITI-41] Provide and Register Document Set – b in the role of the IHE ITI
     * Document Source actor
     *
     * @param submissionSetMetadata The information in this object will be used to
    ...
    public Response setDeprecated(String idOfOriginDocument, String messageId, String xmlAssertion) throws Exception {
        log.debug("set document deprecated");
        AffinityDomain affinityDomain = getAffinityDomain();
        setDefaultKeystoreTruststore(affinityDomain.getRepositoryDestination());

        ProvideAndRegisterDocumentSet documentSet = getTxnData();
        ...
        registerDocumentSet.getAssociations().add(statusUpdate);
