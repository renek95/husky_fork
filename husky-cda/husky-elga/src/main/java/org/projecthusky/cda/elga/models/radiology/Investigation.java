...
public class Investigation extends POCDMT000040Section {

    public Investigation(Float dose, String effectiveTime) {
        super.getClassCode().add("DOCSECT");
        super.getTemplateId().add(createHl7TemplateIdFixedValue("1.2.40.0.34.11.5.2.5"));
        super.setCode(createHl7CodeFixedValue("55111-9",
                LOINC.getCodeSystemId(),
                LOINC.getCodeSystemName(),
                "Current imaging procedure descriptions"));

        if (dose != null && dose > 0) {
            super.getEntry().add(new Dose(dose, effectiveTime));
        }
    }
